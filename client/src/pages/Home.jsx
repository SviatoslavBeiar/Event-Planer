...
function Home() {
    ...
    const EmptyState = (
        <Center>
            <VStack h={'60vh'} alignItems={'center'} justifyContent={'center'} spacing={6}>
                <Heading size="md">
                    {tabIndex === 0 ? 'Немає постів від користувачів, на яких ви підписані' : 'Поки що немає постів'}
                </Heading>
                <Image src={svg} h={'40vh'} alt="No data" />
                {tabIndex === 0 && (
                    <Text color="gray.500">Підпишіться на когось або відкрийте вкладку “Всі пости”.</Text>
                )}
            </VStack>
        </Center>
    )
    ...
    const ErrorState = (
        <Center h="40vh">
            <VStack>
                <Heading size="sm" color="red.500">Помилка</Heading>
