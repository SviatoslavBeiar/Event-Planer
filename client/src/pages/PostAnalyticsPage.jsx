...
export default function PostAnalyticsPage() {
    ...
    const isOrganizer = user && post && Number(user.id) === Number(post.userId) && user.role === 'ORGANIZER';
    if (!isOrganizer) {
    ...
    return (
        <Box maxW="lg" mx="auto" mt={8} p={4}>
            <Heading size="lg" mb={4}>{post.title} — Analytics</Heading>

            <AnalyticsPanel postId={post.id} days={7} />

            <Box mt={4} display="flex" justifyContent="flex-end">
                <Button as={RouterLink} to={`/post/${post.id}`} variant="ghost">Back to event</Button>
            </Box>
        </Box>
    );
}
